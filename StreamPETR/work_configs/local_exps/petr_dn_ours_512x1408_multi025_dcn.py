...
model = dict(
    ...
    img_backbone=dict(
        ...
        pretrained='data/pretrained_models/resnet50-0676ba61.pth'),
    img_neck=dict(
...
data = dict(
    ...
    train=dict(
        ...
        use_valid_flag=True,
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'StreamPETR_temporal_infos_train.pkl', classes=class_names, modality=input_modality),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.25), # change to 0.25 may not converge for original PETR
        }),
    weight_decay=0.01)


optimizer_config = dict(type='Fp16OptimizerHook', loss_scale='dynamic', grad_clip=dict(max_norm=35, norm_type=2, error_if_nonfinite=False))
# learning policy
lr_config = dict(
...
work_dir = "work_dirs/petr_dn_ours_512x1408_multi025_dcn"