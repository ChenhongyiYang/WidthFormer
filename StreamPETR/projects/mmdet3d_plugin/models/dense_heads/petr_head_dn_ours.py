...
def pos2posemb1d(pos, num_pos_feats=128, temperature=10000):
    scale = 2 * math.pi
    pos = pos * scale
    dim_t = torch.arange(num_pos_feats, dtype=torch.float32, device=pos.device)
    dim_t = temperature ** (2 * (dim_t // 2) / num_pos_feats)
    _pos = pos[..., 0, None] / dim_t
    posemb = torch.stack((_pos[..., 0::2].sin(), _pos[..., 1::2].cos()), dim=-1).flatten(-2)
    return posemb
...
def pos2posemb4d(pos, num_pos_feats=128, temperature=10000):
    ...
    dim_t = torch.arange(num_pos_feats, dtype=torch.float32, device=pos.device)
    dim_t = temperature ** (2 * (dim_t // 2) / num_pos_feats)
    pos_0 = pos[..., 0, None] / dim_t
    pos_1 = pos[..., 1, None] / dim_t
    pos_2 = pos[..., 2, None] / dim_t
    pos_3 = pos[..., 3, None] / dim_t

    pos_0 = torch.stack((pos_0[..., 0::2].sin(), pos_0[..., 1::2].cos()), dim=-1).flatten(-2)
    pos_1 = torch.stack((pos_1[..., 0::2].sin(), pos_1[..., 1::2].cos()), dim=-1).flatten(-2)
    pos_2 = torch.stack((pos_2[..., 0::2].sin(), pos_2[..., 1::2].cos()), dim=-1).flatten(-2)
    pos_3 = torch.stack((pos_3[..., 0::2].sin(), pos_3[..., 1::2].cos()), dim=-1).flatten(-2)
    posemb = torch.cat((pos_0, pos_1, pos_2, pos_3), dim=-1)
    return posemb


@HEADS.register_module()
class PETRHeadDNOurs(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadDNOurs):
        ...
        super(PETRHeadDNOurs, self).__init__(num_classes, in_channels, init_cfg=init_cfg)
    ...
    def _init_layers(self):
        ...
        self.depth_net = torch.nn.Conv2d(self.embed_dims, self.depth_num, kernel_size=1, padding=0)
    ...
    def position_embeding(self, depth_pred, data, memory_centers, topk_indexes, img_metas, scale=10.0):
        ...
        with torch.no_grad():
            ...
            r = torch.sqrt(x ** 2 + y ** 2)  # [B, N, M]
            cos = x[..., -1] / (r[..., -1] + eps)  # [B, M]
            sin = y[..., -1] / (r[..., -1] + eps)  # [B, M]
            z = coords3d[..., 2]  # [B, N, D]

            r_embed = pos2posemb1d(r[..., None] * scale, num_pos_feats=self.pe_dims)
            cos_embed = pos2posemb1d(cos[..., None] * scale, num_pos_feats=self.pe_dims)
            sin_embed = pos2posemb1d(sin[..., None] * scale, num_pos_feats=self.pe_dims)
            z_embed = pos2posemb1d(z[..., None] * scale, num_pos_feats=self.pe_dims)

        depth_pred = depth_pred[..., None]

        r_embed = (r_embed * depth_pred).sum(dim=-2)
        z_embed = (z_embed * depth_pred).sum(dim=-2)
        polar_embed = torch.cat(
            (r_embed, cos_embed, sin_embed, z_embed), dim=-1
        ) # [B, M, 4 * 128]

        coords_position_embeding = self.position_encoder(polar_embed)
    ...
    def _get_query_embeds(self, ref_pts, scale=10):
        x = ref_pts[..., 0] - 0.5
        y = ref_pts[..., 1] - 0.5
        z = ref_pts[..., 2]
        ...
        cos = x / (radius + 1e-5)
        sin = y / (radius + 1e-5)
        polar_coor = torch.stack((radius, cos, sin, z), dim=-1)
        return self.query_embedding(pos2posemb4d(polar_coor * scale, num_pos_feats=self.pe_dims))
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadDNOurs:
            convert_dict = {
    ...
    def forward(self, memory_center, img_metas, topk_indexes, **data):
        ...
        depth_pred = (
            self.depth_net(x.view(B * N, C, H, W))
            .reshape(B, N, D, H, W)
            .permute(0, 1, 3, 4, 2)
            .reshape(B, num_tokens, D)
        )
        depth_pred = nn.functional.relu(depth_pred)
        # softmax
        depth_pred = topk_gather(depth_pred, topk_indexes)

        memory = x.permute(0, 1, 3, 4, 2).reshape(B, num_tokens, C)
        memory = topk_gather(memory, topk_indexes)

        pos_embed, cone = self.position_embeding(depth_pred, data, memory_center, topk_indexes, img_metas)
        ...
        query_pos = self._get_query_embeds(reference_points)
