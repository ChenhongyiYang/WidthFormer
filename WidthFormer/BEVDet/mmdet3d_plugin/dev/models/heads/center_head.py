...
class SeparateHeadDev(BaseModule):
    ...
    def __init__(self,
        ...
        super(SeparateHeadDev, self).__init__(init_cfg=init_cfg)
...
@HEADS.register_module()
class DCNSeparateHeadDev(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(DCNSeparateHeadDev, self).__init__(init_cfg=init_cfg)
        ...
        self.task_head = SeparateHeadDev(
            in_channels,
            heads,
            head_conv=head_conv,
    ...
    def forward(self, x):
        ...
        if not self.with_cp:
            ...
            center_feat = checkpoint(self.feature_adapt_cls, x)
            reg_feat = checkpoint(self.feature_adapt_reg, x)
...
@HEADS.register_module()
class CenterHeadDev(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(CenterHeadDev, self).__init__(init_cfg=init_cfg)
        self.with_cp = with_cp
        ...
        for num_cls in num_classes:
            ...
            separate_head.update(
                in_channels=share_conv_channel, heads=heads, num_cls=num_cls, with_cp=with_cp)
