...
class BEVDet(CenterPoint):
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        x = self.img_view_transformer([x] + img[1:]) # [B, 1 * numC_Trans, BEV_H, BEV_W] , [4, 64, 128, 128]
    ...
    def forward_train(self,
        ...
        points: None
        img_metas: list, length: batch size, 
            flip: bool
            pcd_horizontal_flip: bool
            pcd_vertical_flip: bool
            box_mode_3d: mode of bbox, Box3DMode.LIDAR
            box_type_3d: class of bbox, mmdet3d.core.bbox.structures.lidar_box3d.LiDARInstance3DBoxes
            pcd_trans: [3,] 
            sample_idx: sample id, str
            pcd_scale_factor: float
            pcd_rotation: [3, 3], 3x3 rotation matrix
            pts_filename: LIDAR sample file name
            transformation_3d_flow: list of string, e.g., ['R', 'S', 'T', 'HF']
            img_info: dict
                keys: CAM_FRONT, CAM_FRONT_RIGHT, CAM_FRONT_LEFT, CAM_BACK, CAM_BACK_LEFT, CAM_BACK_RIGHT
                for each of the keys:
                    type: the key, e.g., CAM_FRONT
                    data_path: str, image path
                    sample_data_token: a token, does not matter
                    sensor2ego_translation: [3,]
                    sensor2ego_rotation: [3,]
                    ...
                    cam_intrinsic: [3,3]
        gt_bboxes_3d: list, length: batch size, each entry is an LiDARInstance3DBoxes instance, with a [M, 9] tensor in it, M is the number of instance
        gt_labels_3d: list, length: batch size, each entry is an [M,] tensor with class labels in it
        gt_labels: None
        gt_bboxes: None
        img_inputs: list, length 6 
            0: [B, n_cam, 3, H, W], multi-view image input
            1: [B, n_cam, 3, 3] rots
            2: [B, n_cam, 3] trans 
            3: [B, n_cam, 3, 3] intrins
            4: [B, n_cam, 3, 3] post_rots, what is post: camera coor to ego coor? 
            5: [B, n_cam, 3] post_trans
        proposals: None,
        gt_bboxes_ignore: None
        """
