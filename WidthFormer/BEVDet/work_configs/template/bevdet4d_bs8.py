...
model = dict(
    type='BEVDetSequentialES_NewForward',
    aligned=True,
    detach=True,
    before=True,
    img_backbone=dict(
        pretrained='pretrained_models/resnet50-19c8e357.pth',
        type='ResNet_CorrectCP',
        depth=50,
        num_stages=4,
        out_indices=(2, 3),
        frozen_stages=-1,
    ...
    img_view_transformer=dict(type='ViewTransformerLiftSplatShoot',
                              ...
                              use_bev_pool=True,
                              accelerate=False),
    img_bev_encoder_backbone = dict(type='ResNetForBEVDet_CorrectCP',  numC_input=128,
                                    num_channels=[128, 256, 512]),
    img_bev_encoder_neck = dict(type='FPN_LSS',
                                in_channels=numC_Trans*8+numC_Trans*2,
                                out_channels=256),
    pre_process = dict(type='ResNetForBEVDet_CorrectCP',numC_input=numC_Trans,
...
test_pipeline = [
dict(type='LoadMultiViewImagesFromFiles_BEVDet', data_config=data_config,
     sequential=True, aligned=True, trans_only=False),
...
lr_config = dict(
    ...
    warmup_ratio=0.001,
    step=[16, 22])
runner = dict(type='AmpEpochBasedRunner', max_epochs=23)
